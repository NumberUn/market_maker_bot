...
class DB:
    ...
    def save_arbitrage_possibilities(self, deal, precised_sz, ts_send, ap_id, buy_id, sell_id, inner_ping):
        message = {
            ...
            'datetime': datetime.utcnow(),
            'ts': ts_send,
            'buy_exchange': deal['ex_buy'],
            'sell_exchange': deal['ex_sell'],
            'symbol': deal['coin'],
            'buy_order_id': buy_id,
            'sell_order_id': sell_id,
            'max_buy_vol_usd': round(precised_sz * deal['buy_px'], 2),
            'max_sell_vol_usd': round(precised_sz * deal['sell_px'], 2),
            'expect_buy_price': deal['buy_px'],
            'expect_sell_price': deal['sell_px'],
            'expect_amount_usd': round(precised_sz * deal['buy_px'], 2),
            'expect_amount_coin': precised_sz,
            'expect_profit_usd': round(precised_sz * deal['buy_px'] * deal['profit'], 2),
            'expect_profit_relative': round(deal['profit'], 6),
            'expect_fee_buy': deal['client_buy'].taker_fee,
            'expect_fee_sell': deal['client_sell'].taker_fee,
            'time_parser': 0,
            'time_choose': inner_ping,
            'chat_id': 12345678,
            'bot_token': 'Placeholder',
            'status': 'Processing',
    ...
    def save_order(self, order_id, deal, ap_id, resp, side, size, env):
        message = {
            ...
            'order_place_time': resp['create_order_time'] if resp else 'default',
            'env': env,
        }
